...
var MovieListModel = Backbone.Model.extend({
	...
	initialize: function(renderView, options) {
        ...
        this.userListArray = [gS.watchListID, gS.favoriteListID, gS.seenListID];
        ...
        if (options) {
            ...
            this.limit = this.userListArray.indexOf(options.listID) === -1 ? this.maxLimit : 50;
    ...
    render: function(callback) {
		...
		if(list) {
			self.model.getListData(function(list) {
                ...
                if(list[0] && !list[0].isUser) {

                    var type = listTypes[parseInt(list[0].typeID)],
                        notInCommon = [];


                    var inCommon = list.filter(function (value, i, array) {

                        return value[type] === "1" ? true : ( notInCommon.push(value), false );
					...
					this.$('#list-movies')
						.append( this.inCommon.render().el )
						.append( this.notInCommon.render().el );

                } else {

					this.ownList = new ListSection({ collection: list });

					this.$('#list-movies')
						.append( this.ownList.render().el );
...
var ListSection = Backbone.View.extend({
	...
	addOne: function (listItemData) {

		var listItem = new ListItem({ model: listItemData });
...
var ListHeadingView = Backbone.View.extend({
	className: 'line',
	...
	render: function () {
		...
		this.$el.html('<h1 class="list-grid-heading">'+ this.count +' '+ this.heading +'</h1>');
...
var ListItem = Backbone.View.extend({
	...
	render: function () {
		...
		this.el.src = Api.appSettings.cdn +"/posters/"+ this.model.toJSON().moviePublishedID +"_poster.jpg";
