...
- (void) silentTweet:(CDVInvokedUrlCommand*)command {
    // arguments: callback, tweet text, url attachment, image attachment
    NSMutableString *tweetText = [NSMutableString stringWithString:[command.arguments objectAtIndex:0]];
    NSMutableString *urlAttach = [command.arguments objectAtIndex:1];
    NSString *errorMessage;
    BOOL ok = YES;
    
    if(tweetText != nil) {
        //NSUInteger length = [tweetText length];
        //if(length > 139) {
        //    ok = NO;
        ...
        if(urlAttach != nil) {
            ...
            //} else {
                ...
                [tweetText appendString:urlAttach];
            //}
        }
    }
    else {
        ok = NO;
        errorMessage = @"No tweet text was found";
    }
    
    if(!ok) {
        NSString *jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:errorMessage]
        ...
        [account requestAccessToAccountsWithType:accountType options:nil completion:^(BOOL granted, NSError *error) {
            ...
            if (granted == YES) {
                ...
                if ([arrayOfAccounts count] > 0) {
                    ...
                    [postRequest performRequestWithHandler: ^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error) {
                        ...
                        if ([JSON objectForKey:@"errors"]) {
                            ...
                            jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                                           messageAsString:@"Tweet was successful"]
                    ...
                    errorMessage = @"Twitter account not found on this device.";
                    NSString *jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:errorMessage] toErrorCallbackString:command.callbackId];
                    [self performCallbackOnMainThreadforJS:jsResponse];
                }
            } else {
                errorMessage = @"Access not granted for Twitter.";
                NSString *jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:errorMessage] toErrorCallbackString:command.callbackId];
                [self performCallbackOnMainThreadforJS:jsResponse];
            }
        }];
...
- (void) silentFBPost:(CDVInvokedUrlCommand*)command {
    NSMutableString *postText = [NSMutableString stringWithString:[command.arguments objectAtIndex:0]];
    NSMutableString *postAttach = [command.arguments objectAtIndex:1];
    ...
    NSDictionary *options = @{
        ACFacebookAppIdKey: @"908540922496603",
    ...
    [accountStore requestAccessToAccountsWithType:facebookAccountType options:options completion:^(BOOL granted, NSError *e) {
         if (granted) {
             ...
             NSDictionary *parameters = @{@"message": postText, @"link": postAttach};
...
- (void) getDeviceID:(CDVInvokedUrlCommand*)command {
    NSString *deviceID = [UIDevice currentDevice].uniqueAppInstanceIdentifier;
    NSString *jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_OK
